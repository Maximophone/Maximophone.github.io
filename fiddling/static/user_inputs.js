...
export class UserInputs {
    ...
    static press_key(k){
	socket.emit("input", {type: "keydown", key: k.keyCode})
	_pressed_keys[k.keyCode] = true
    }
    static release_key(k){
	socket.emit("input", {type: "keyup", key: k.keyCode})
	_pressed_keys[k.keyCode] = false
    }
    static move_mouse(e){
	socket.emit("input", {type: "mousemove", x: 2*e.offsetX/c.width - 1, y: -2*e.offsetY/c.height + 1})
