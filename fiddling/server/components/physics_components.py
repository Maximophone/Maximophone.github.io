...
class ShipPhysicsComponent(PhysicsComponent):
    def update(self, dt):
        ...
        if engine.accelerate:
            self.vx = cap_abs(self.vx + dt*engine.delta_v*cos, math.abs(engine.max_v*cos))
            self.vy = cap_abs(self.vy + dt*engine.delta_v*sin, math.abs(engine.max_v*sin))
        elif engine.deccelerate:
            self.vx = cap_abs(self.vx - dt*engine.delta_v*cos, math.abs(engine.max_v*cos))
            self.vy = cap_abs(self.vy - dt*engine.delta_v*sin, math.abs(engine.max_v*sin))
        if engine.strafe_right:
            self.vx = cap_abs(self.vx + dt*engine.delta_v*cos_left, math.abs(engine.max_v*cos_left))
            self.vy = cap_abs(self.vy + dt*engine.delta_v*sin_left, math.abs(engine.max_v*sin_left))
        elif engine.strafe_left:
            self.vx = cap_abs(self.vx + dt*engine.delta_v*cos_right, math.abs(engine.max_v*cos_right))
            self.vy = cap_abs(self.vy + dt*engine.delta_v*sin_right, math.abs(engine.max_v*sin_right))

        # FRICTION
        if not (engine.accelerate or engine.deccelerate or engine.strafe_left or engine.strafe_right):
            if self.v() > 0:
                if self.vx > 0:
                    self.vx = math.max(self.vx - dt*V_FRICTION, 0)
                else:
                    self.vx = math.min(self.vx + dt*V_FRICTION, 0)
                if self.vy > 0:
                    self.vy = math.max(self.vy - dt*V_FRICTION, 0)
                else:
                    self.vy = math.min(self.vy + dt*V_FRICTION, 0)
        ...
        self.entity.position.x += self.vx
        self.entity.position.y += self.by
